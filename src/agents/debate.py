...
## 五、裁决倾向（Bull / Bear / 中性）+ 概率化理由
## 六、事后可验证清单 ⭐
**这是辩论质量的核心交付物**——把 Bull 第三章"可证伪信号"+ Bear 第三章"可证伪信号"+
双方各自的"胜出条件"统一收编, 输出一份"未来 6 个月该观察什么"的清单：

| # | 观察事件 / KPI | 阈值 / 触发条件 | 验证哪一方 | 观察窗口 | 数据源 |
|---|---|---|---|---|---|
| 1 | Q1 客户 top1 占比 | > 35% | Bear 对 | 招股后 90 天 | 季报 |
| 2 | 具身智能业务占比 | 提升至 ≥ 18% | Bull 对 | 招股后 180 天 | 半年报 |
| ... | ... | ... | ... | ... | ... |

清单要求：5-10 条；每条必须可量化、可观测、有明确数据源；
**这份清单会被 PostmortemAgent 在 6 月后实测打分**——不能写无法验证的内容（如"管理层执行力强")。

## 七、关键不确定性（需在风控环节进一步评估的事项）

输出 1200-1800 字（增加事后清单后允许更长）。
...
- 裁决倾向必须给出**概率化**的理由（如"Bear 框架仅需单点失效, Bull 需 4 假设同时成立, 概率乘积低"），
  不要写"Bear 更稳"等情绪化表述。
- 第六章"事后可验证清单"必须**严格从 Bull/Bear 双方的'可证伪信号'和'胜出条件'中提取/合并**,
  不引入新假设。
